...
def find_neighbours(simplices, points_2d):
...
def plot_delaunay(simplices, points, neighbours, path=None):
    points_2d = points[:, :2]
    ...
    ax1.triplot(points_2d[:, 0], points_2d[:, 1], simplices)
    ax1.scatter(
    ...
    # Determine points in the specified range
    in_range = (
        (points_2d[:, 0] >= 30)
        & (points_2d[:, 0] <= 60)
        & (points_2d[:, 1] >= 30)
    ...
    # Determine simplices that have at least one vertex in the specified range
    simplices_to_plot = [
        simplex for simplex in simplices if any(in_range[vertex] for vertex in simplex)
    ]

    # Neighbour analysis for the filtered points
    nine_neighbours = in_range & np.array([len(nbrs) == 9 for nbrs in neighbours])
    eight_neighbours = in_range & np.array([len(nbrs) == 8 for nbrs in neighbours])
    seven_neighbours = in_range & np.array([len(nbrs) == 7 for nbrs in neighbours])
    five_neighbours = in_range & np.array([len(nbrs) == 5 for nbrs in neighbours])
    four_neighbours = in_range & np.array([len(nbrs) == 4 for nbrs in neighbours])
    three_neighbours = in_range & np.array([len(nbrs) == 3 for nbrs in neighbours])

    ax2 = fig.add_subplot(1, 2, 2, projection="3d")
    for simplex in simplices_to_plot:
        ax2.plot(
            points[simplex, 0],
            points[simplex, 1],
            points[simplex, 2],
