...
def annealing(n, amp, surface, atoms, start_temp, cooling_rate, end_temp):
    ...
    local_minimisation.run(steps=10000, fmax=0.015)
...
def sine_surfaces(amplitude, density):
    return (amplitude, Surface(lambda x,y: amplitude*sp.sin(2 * sp.pi * x / 30) * sp.sin(2 * sp.pi * y / 30), n=density))

# surfaces = [sine_surfaces(a) for a in np.arange(1, 10.5, 0.5)]
surfaces = [sine_surfaces(5, density) for density in densities]
file = f"results-surfaces/surfaces-{seed}.txt"
...
def launch_parallel(nstream, surface, start_temp, cooling_rate, end_temp):
    ...
    with paropen(file, "a") as resfile:
        print(n, amp, surf.density, surf.n, r0, start_temp, cooling_rate, end_temp, energy, file=resfile)
    # tri, points, points_2d, neighbours, amplitude = make_delaunay_triangulation(f"trajectories-surfaces/{seed}/bfgs-{n}-{amp}-{surf.n}-{start_temp}-{cooling_rate}-{end_temp}.traj")
    # print_neighbour_counts(points, neighbours)
    # plot_delaunay(tri, points, points_2d, neighbours, amplitude, f"{n}-{amp}-{surf.n}-{start_temp}-{cooling_rate}-{end_temp}")


def pool_handler():
    p = Pool(n_processes)
    with open(file,"a") as resfile:
        print(f"Description: seed={seed} density adjusted r0, {n_streams} streams", file=resfile)
